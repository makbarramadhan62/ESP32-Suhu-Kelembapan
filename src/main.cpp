...
void loop()
{
  ...
  else if (data.temperature >= 27 && !msgSentTemp)
  {
    String reply = "";
  ...
  if ((data.temperature <= 18 || data.temperature >= 27) && msgSentTemp && !timerTemp)
  {
    previousTimeTemp = currentTime;
    timerTemp = true;
  }
  else if (data.temperature >= 18 && data.temperature <= 27 && msgSentTemp && timerTemp)
  {
    timerTemp = false;
    msgSentTemp = false;
  }
  ...
  if (myBot.getNewMessage(tMessage))
  {
    ...
    if (tMessage.messageType == CTBotMessageText)
    {
      ...
      else if (tMessage.text.equalsIgnoreCase("/get_temp"))
      {
        ...
        reply += String(data.temperature) + " degC";
        myBot.sendMessage(tMessage.sender.id, reply);
      }
